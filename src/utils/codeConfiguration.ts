...
export class CodeConfigManager {
  ...
  static createDefaultConfig(language: Language, _category: QuestionCategory): CodeConfig {
    ...
    return {
      ...
      runtime: (defaultRuntime?.id as 'node' | 'python' | 'sql' | 'dart') || this.getLanguageDefaultRuntime(language),
  ...
  private static getLanguageDefaultRuntime(language: Language): 'node' | 'python' | 'sql' | 'dart' {
    const defaults: Record<Language, 'node' | 'python' | 'sql' | 'dart'> = {
      javascript: 'node',
      typescript: 'node',
      react: 'node',
      reactNative: 'node',
  ...
  static validateCodeConfig(
    ...
    if (category === 'logic') {
      ...
      if (!config.runtime) {
        ...
        // Check if runtime is valid for backend
