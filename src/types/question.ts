  ...
  switch (question.type) {
    ...
    case 'trueFalse':
      if (typeof question.correctAnswer !== 'boolean') {
        errors.push('Boolean correct answer required for true/false');
      }
      break;
...
export const getQuestionTypeConfig = (type: QuestionType): QuestionTypeConfig => {
  const configs: Record<QuestionType, QuestionTypeConfig> = {
    ...
    fillInTheBlank: {
      ...
      requiredFields: ['category', 'codeTemplate', 'blanks']
    },
    codeDebugging: {
      type: 'codeDebugging',
      name: 'Code Debugging',
      ...
      autoGradeable: true,
      supportedCategories: ['logic'],
      requiredFields: ['category', 'buggyCode', 'solutionCode']
    }
  };
